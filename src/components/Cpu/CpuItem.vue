...
export default {
    ...
    methods: {  
        async addToCart() {
            await axios.post('https://nicepc-1-s2894705.deta.app/cart/', {'product_id': this.cpu_item.id, 'price': this.cpu_item.price, 'name': this.cpu_item.name}, {headers: {'Authorization': `Bearer ${localStorage.getItem('token')}`}})
            .then((res) => {
                if (res.status === 200) {
